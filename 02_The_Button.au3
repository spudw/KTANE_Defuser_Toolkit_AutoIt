...
Global Const $THEBUTTON_MSG_HOLD_BUTTON = "Have the Defuser Hold Down the Button"
...
Func _TheButton_EvaluateTheButton()
	...
	If StringInstr($sTheButtonInput,"D") And Not $sTheButtonIndicator Then
		If MsgBox($MB_YESNO + $MB_ICONERROR, "Inspect Bomb", "Is there more than One Battery?") = $IDYES Then
			Return $THEBUTTON_MSG_PRESS_AND_RELEASE	;If there is more than 1 battery on the bomb and the button says "Detonate", press and immediately release the button.
		Else
			Return 0
		EndIf
	EndIf
